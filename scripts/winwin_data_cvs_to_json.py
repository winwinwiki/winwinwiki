...
def main(argv):
    ...
    if es_url:
        format = "dump"
    process_csv(input_file, output_file, format, es_url)

#Process CSV File
def process_csv(file, json_file, format, es_url):
    csv_rows = []
    upload_command = None
    if es_url:
    ...
    with open(file) as csvfile:
        ...
        for row in reader:
            ...
            for i in range(len(title)):
                field_name = title[i].strip()
                field_value = row[field_name]
                field_name = field_name.strip('\xef\xbb\xbf')
...
def write_json(data, json_file, format):
    with open(json_file, "w") as f:
        for entry in data:
            if format == "pretty":
                f.write(json.dumps(entry, sort_keys=True, indent=4, separators=(',', ': '),encoding="utf-8",ensure_ascii=False))
            else:
